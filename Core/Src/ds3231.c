...
/**
 * @brief Get current time and date from RTC in one simple call
 * @param timeDate Pointer to RTC_TimeDate structure to store the results
 * @retval None
 * @note This function reads all time/date values from DS3231 over I2C
 */
void RTC_GetCurrentTime(RTC_TimeDate *timeDate) {
	...
	// Read all time and date values from RTC
	timeDate->hours = DS3231_GetHour();
	timeDate->minutes = DS3231_GetMinute();
	timeDate->seconds = DS3231_GetSecond();
	timeDate->dayOfWeek = DS3231_GetDayOfWeek();
	timeDate->date = DS3231_GetDate();
	timeDate->month = DS3231_GetMonth();
	timeDate->year = DS3231_GetYear();
}

/**
 * @brief Set RTC time and date from structure (e.g., from PC)
 * @param timeDate Pointer to RTC_TimeDate structure containing the time to set
 * @retval None
 * @note This function writes all time/date values to DS3231 over I2C
 */
void RTC_SetCurrentTime(RTC_TimeDate *timeDate) {
	...
	// Set time
	DS3231_SetFullTime(timeDate->hours, timeDate->minutes, timeDate->seconds);
	
	// Set date
	DS3231_SetFullDate(timeDate->date, timeDate->month, timeDate->dayOfWeek, timeDate->year);
}

/**
 * @brief Set RTC time and date with individual parameters
 * @param hours Hour in 24h format, 0 to 23
 * @param minutes Minute, 0 to 59
 * @param seconds Second, 0 to 59
 * @param date Day of month, 1 to 31
 * @param month Month, 1 to 12
 * @param year Year, 2000 to 2199
 * @param dayOfWeek Days since last Sunday, 1 to 7
...
void RTC_SetTime(uint8_t hours, uint8_t minutes, uint8_t seconds,
	...
	// Set time
	DS3231_SetFullTime(hours, minutes, seconds);
